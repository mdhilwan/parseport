...
export const HttpActions = {
    ...
    async DeactivateUser(userEmail) {
        const res = await doFetchPost(DEACTIVATE_USER, { data: { userEmail: userEmail } })
        return { res: await res.json(), email: userEmail }
    },
    async ActivateUser(userEmail) {
        const res = await doFetchPost(ACTIVATE_USER, { data: { userEmail: userEmail } })
        return { res: await res.json(), email: userEmail }
    },
    async DeleteUser(userEmail) {
        const res = await doFetchPost(DELETE_USER, { data: { userEmail: userEmail } })
        return { res: await res.json(), email: userEmail }
    },
    async AddNewUser({ data: { company, companyAddress, companyNumber, userEmail } }) {
        const res = await doFetchPost(ADD_NEW_USER, { data: { company: company, companyAddress: companyAddress, companyNumber: companyNumber, userEmail: userEmail } })
        return { res: await res.json(), email: userEmail }
    }
}